...
with st.container(border=True):
    with st.form("form_novo_catalogo", clear_on_submit=True):
        ...
        if st.form_submit_button("➕ Adicionar ao Catálogo", type="primary", use_container_width=True):
            if nome_cat:
                supabase.table("catalogo_servicos").insert({"nome": nome_cat, "valor_padrao": valor_cat}).execute()
                st.cache_data.clear()
                st.toast("✅ Serviço adicionado ao catálogo!")
                time.sleep(0.5)
                st.rerun()
...
if df_catalogo.empty:
    ...
    for _, r in df_catalogo.iterrows():
        with st.container(border=True):
            ...
            with rc1: st.write(f"🛠️ **{r['nome']}**")
            with rc2: st.markdown(f"<span style='color:green;'>R$ {r['valor_padrao']:,.2f}</span>", unsafe_allow_html=True)
            with rc3:
                if st.button("🗑️", key=f"del_cat_{r['id']}"):
                    supabase.table("catalogo_servicos").delete().eq("id", r['id']).execute()
                    st.cache_data.clear()
                    st.toast("🗑️ Removido do catálogo!")
                    time.sleep(0.5)
                    st.rerun()
